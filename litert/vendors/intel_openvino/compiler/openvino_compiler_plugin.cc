...
LiteRtStatus LiteRtCompilerPluginCompile(
  ...
  try {
    ...
    // same serialized blob is returned for every partition; the dispatcher
    // selects its subgraph and resolves the subgraph's weights against the
    // shared pool.
    //
    // Enabled only when EVERY partition targets the SAME shareable backend (all
    // GPU or all NPU); mixed devices or a non-shareable backend (CPU) fall back
    ...
    // Authoritative BufferId -> byte offset in the contiguous pool, built in
    ...
    for (int partition_idx = 0; partition_idx < num_partitions;
      ...
      if (share_npu) {
        // NPU shared path: turn on NPUW/CWAI so export_model emits a
        // weightless blob whose constants are referenced by
        // WeightlessCacheAttribute bin_offset instead of baked in.
        context.ConfigureForNpuWeightSharing(
            context.ResolveMoeHostRouterEnable(ov_model));
      }
    ...
    for (int partition_idx = 0; partition_idx < num_partitions;
      ...
      ov::save_model(ov_model, graph_name + ".xml");
    ...
    if (share_weights) {
      ...
      // partition's call at byte_code_idx 0 (see
      // LiteRtGetCompiledResultCallInfo). This matches the upstream reference
      // (one shared blob) and, critically, stores the ~multi-GB buffer pool
      // exactly once in the .tflite -- copying it into N per-partition slots
      // would N-x the file and defeat sharing.
